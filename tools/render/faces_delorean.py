...
def face_delorean(c, hh, mm, ss, mday, mon, year, wday, temp_f, sunrise, sunset):
    ...
    c.fillScreen(CASE)
    c.drawSmoothCircle(120, 120, 119, FRAME, CASE)
    c.drawSmoothCircle(120, 120, 117, FRAME, CASE)
    ...
    well(35, 108, 34, 24)
    digits("%02d" % hh, 38, 111, 13, 18, 3, 3, RED, RED_OFF)
    well(73, 108, 34, 24)
    digits("%02d" % mm, 76, 111, 13, 18, 3, 3, RED, RED_OFF)
    well(111, 108, 34, 24)
    digits("%02d" % ss, 114, 111, 13, 18, 3, 3, RED, RED_OFF)
    well(149, 108, 56, 24)
    digits("%04d" % year, 153, 111, 11, 18, 3, 2, RED, RED_OFF)

    tab("WEEK", 50, 138, 42)
    tab("DAY", 92, 138, 34)
    tab("MONTH", 134, 138, 42)
    tab("DAYLIGHT", 185, 138, 56)

    well(29, 150, 42, 24)
    letters(WDAYS[wday], 32, 153, 11, 18, 3, 2, GRN, GRN_OFF)
    well(75, 150, 34, 24)
    digits("%02d" % mday, 78, 153, 13, 18, 3, 3, GRN, GRN_OFF)
    well(113, 150, 42, 24)
    letters(MONTHS[mon], 116, 153, 11, 18, 3, 2, GRN, GRN_OFF)
    well(159, 150, 52, 24)
    dl = sunset - sunrise
    digits("%02d%02d" % (dl // 60, dl % 60), 162, 153, 11, 18, 3, 2, GRN, GRN_OFF)

    show_rise = (ss // 5) % 2 == 0
    sun_val = sunrise if show_rise else sunset
    tab("SUNRISE" if show_rise else "SUNSET", 84, 180, 58)
    well(55, 192, 58, 22)
    digits("%02d%02d" % (sun_val // 60, sun_val % 60), 58, 195, 12, 16, 3, 2, AMB, AMB_OFF)

    tab("TEMP", 156, 180, 58)
    well(127, 192, 58, 22)
    digits("%3d" % temp_f, 131, 195, 12, 16, 3, 3, AMB, AMB_OFF)
